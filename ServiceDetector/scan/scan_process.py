...
class ScanProcess(object):
    ...
    def set_options(self, string_options_from_parsing):
        ...
        self.logger.print_debug("[PSScanProcess]options: {0}".format(self.options_str))
    ...
    def run(self):
        ...
        self.logger.print_debug("[PSScanProcess]nmap:command>>", self.command_line)
    ...
    def print_scan(self):
        ...
        try:
            ...
            self.logger.print_info("[MultiScanManager]Exception raised while parsing scan:{0}".format(e.msg))
            print_error(RESULT_PARSE_ERROR,'{0}'.format(e.msg))
            error_number = len(self.targets)
            self.logger.print_debug("[MultiScanManager]parse error")
            return error_number
