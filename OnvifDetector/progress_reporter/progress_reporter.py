...
LOG_LEVEL = _gv.GL_LOG_LEVEL['progress_reporter']
...
class ProgressReporter(Thread):
    ...
    def run(self):
        while self.is_continue:
            ...
            self.logger.info('[ProgressReporter]Status {0}'.format(_time.strftime('%H:%M:%S', _time.localtime(time_cost))), '[OnvifDetector]')
