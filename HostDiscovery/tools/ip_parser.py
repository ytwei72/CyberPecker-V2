...
class IPParser:
    ...
    def get_nmap_ip_parameter(self,*nmap_target_spec):
        """
        return list of ip set as nmap's target parameter
        """
        target_list = []
        raw_spec = nmap_target_spec
        for target in raw_spec:
            ip_list = [str(ip) for ip in self.iter_nmap_range(target)]
            target_list.extend(ip_list)
        return target_list
    ...
    def is_valid_ip(self, target_spec):
        """ check if ip address string is valid

        :param target_spec:
        :return: True if target_spec is valid ip, else False
        :rtype: bool
        """
        if '/' in target_spec:
            ...
            try:
                ...
                return False
            return True if net.version == 4 else False

        elif ':' in target_spec:
            # nmap only currently supports IPv6 addresses without prefixes.
            try:
                IPAddress(target_spec)
                return True
            except (AddrFormatError,):
                return False
        else:
            ...
            except (TypeError, ValueError, AddrFormatError) as e:
                return False
            return True
            pass
    ...
    def _parse_nmap_target_spec(self, target_spec):
        if '/' in target_spec:
            ...
            try:
                ...
                print_error(ARG_ERROR,'--target %s : %s' % (str(e),target_spec))
                exit(0)
            ...
            if isinstance(target_spec,list):
                for target_spec_item in target_spec:
                    ...
                    for addr in self._parse_nmap_target_spec(target_spec_item):
                        ...
                        #if addr not in self.tmp_target:
